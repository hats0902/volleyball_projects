...
バレーボールの試合動画・画像から、コート上の選手の骨格（キーポイント）を検出するプロジェクト。
検出した座標データをアナリスト等が分析に活用することを想定し、ダウンロードまで実装をおこなっている。

Live demo: [pose_estimation_model_for_volleyball](https://huggingface.co/spaces/hats0902/pose_estimation_model_for_volleyball)（検出結果の枠をクリックして選手を選択し、その選手だけのキーポイント座標をJSON形式でダウンロードする機能を実装済み）
...
1. **ジャンプして腕を大きく伸ばしている選手を検出できない**：バレーボールはジャンプしてトスを上げる・アタックを打つ・ブロックするといった、空中で腕を頭上に大きく伸ばす動作が頻繁に発生する。
こうした一般的な人物姿勢推定モデルが学習していない特徴的な姿勢を、既存モデルは正しく認識できないことが多かった。特にセッターがトスをする瞬間のフレームでは4/20枚でしか検出できなかった。
...
2025, 2026年開催の国際大会（VNL）の試合を実際に撮影した映像を使用した。試合映像からフレームを抽出し、コートに映っている選手全員（両チーム含む）を対象に、キーポイントとbboxを手動でアノテーションした。1画像に複数の選手が写るため、1枚の画像に対して複数人分のアノテーションが存在する。
...
ベースモデルには`yolo26n-pose`を使用した。上記で作成したデータセットを用いて、Google Colab環境で200 epochのファインチューニングを実施した。

学習過程で得られた重みのうち、検証データに対する評価指標（bboxのmAP50-95とposeのmAP50-95の合計値）が最大となった、200エポック中154エポック目の重みを採用した。（ライブラリ側でbest.ptとして自動で決定される）
...
| --- | --- |
| <img src="images/before_after_4_before.jpg" width="360"> | <img src="images/before_after_4_after.jpg" width="360"> |
| 検出7件（コート上の選手4人＋観客3人） | 検出7件、すべてコート上の選手+監督（観客の誤検出なし） |

| ファインチューニング前 | ファインチューニング後 |
| --- | --- |
| <img src="images/before_after_3_before.jpg" width="360"> | <img src="images/before_after_3_after.jpg" width="360"> |
...
| --- | --- |
| <img src="images/before_after_3b_before.jpg" width="360"> | <img src="images/before_after_3b_after.jpg" width="360"> |

ファインチューニング前は、ネット際で腕を伸ばしてジャンプしている選手にbboxが全く付いていない（手前の選手のみ検出）。ファインチューニング後は同じ選手が骨格付きで検出されている。 -->

いずれの例でも、選手の検出数は同等かそれ以上、観客の誤検出は減少していることがわかった。

### ジャンプ・腕上げ姿勢での検出改善
ここでは選手がジャンプしていたり腕を大きく上に上げているフレームに着目して結果を確認する。
| ファインチューニング前 | ファインチューニング後 |
| --- | --- |
| <img src="images/before_after_5_before.jpg" width="360"> | <img src="images/before_after_5_after.jpg" width="360"> |
| 検出4件（confidence 0.26〜0.57） | 検出6件（confidence 0.73〜0.84）、セットする選手を含め全員を高いconfidenceで検出 |

| ファインチューニング前 | ファインチューニング後 |
| --- | --- |
| <img src="images/before_after_6_before.jpg" width="360"> | <img src="images/before_after_6_after.jpg" width="360"> |
| 検出2件（confidence 0.32, 0.43）。**トスを上げているセッターは検出漏れ** | 検出7件（confidence 0.37〜0.86）。**トスを上げているセッターも検出に成功** |

いずれの画像も、ファインチューニング前は検出数・confidenceともに低く、特に2例目ではトス中のセッター本人やジャンプ直前の選手が検出できていなかった。
一部骨格の座標が間違っていることはあるものの、ファインチューニング後は検出数・confidenceが大きく向上し、選手を正しく検出できている。  
実際にセッターがトスを上げる瞬間フレームを20枚取り出したところ、ファインチューニング前は4枚しかセッターを検出できなかったが、ファインチューニング後は16枚検出できるようになった。

### 検出数の比較

試合動画から60フレームに1回サンプリングした87フレーム（confidenceしきい値0.25）で、ファインチューニング前後を比較した結果。
正解ラベルとの照合はしておらず、観客等の誤検出も1件としてそのままカウントされている参考値である点に注意。
...
採用した154エポック目の重みについて、手動でアノテーションした正解ラベルと照合したmAP等の精度指標を2種類の分割で算出した。
...
- 観客・審判・監督等の誤検出は完全には解消していない。「検出数の比較」の表の通り、ファインチューニング後も1フレームあたり平均7件以上検出されており、これは実際のコート上の選手数（多くの場面で6人前後）を上回る。上記の比較画像で示したフレームではたまたま観客の誤検出が0件になったが、他のフレームでは依然として観客・審判・監督等をコート上の選手と誤って検出することがある。ただし、デモアプリのように**ユーザーが枠を手動でクリックして選手を選び、その選手だけのキーポイントをダウンロードする**という使い方では、誤検出された枠が混ざっていてもユーザー自身が選ばなければ影響しないため、この誤検出をどこまで積極的に解消すべきかは考える余地がある（全選手を自動集計するような用途であれば重要度は上がる）
...
データセット作成におけるフレーム選定やネット座標の特定など、手作業による工程の効率化を目的として、Claude Codeを活用した開発に取り組んでいる。今後はフレーム候補の自動抽出など、データセット作成工程へのAI活用を検討する。
